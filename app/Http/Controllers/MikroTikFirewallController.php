...
class MikroTikFirewallController extends Controller
{
    ...
    public function index(Routeur $routeur, Request $request)
    {
        ...
        // Toujours essayer de récupérer les règles, peu importe le statut en BD
        try {
            switch ($tab) {
                case 'filter':
            ...
            \Log::warning('Erreur récupération règles firewall', ['routeur_id' => $routeur->id, 'error' => $e->getMessage()]);
        }
    ...
    {
        $request->validate([
            'chain' => 'required|in:input,output,forward,INPUT,OUTPUT,FORWARD',
        ...
        Log::info('storeFilter called', ['routeur_id' => $routeur->id, 'statut' => $routeur->statut, 'data' => $data]);

        $success = $this->mikrotik->addFirewallRule($routeur, $data);

        Log::info('storeFilter result', ['routeur_id' => $routeur->id, 'success' => $success]);

        return response()->json([
            'success' => $success,
            'message' => $success ? 'Règle ajoutée' : 'Échec de l\'ajout - vérifiez les logs',
            'debug' => [
                'routeur_id' => $routeur->id,
                'statut' => $routeur->statut,
                'data_sent' => $data
            ]
        ]);
    }
