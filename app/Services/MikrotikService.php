...
class MikrotikService
{
    ...
    {
        try {
            Log::info('Mikrotik getFirewallRules called', ['routeur_id' => $routeur->id]);
            $client = $this->client($routeur);
            ...
            foreach ($resp as $item) {
                ...
                $rules[] = [
                    'id' => $item->getProperty('.id'),
                    'chain' => strtoupper($item->getProperty('chain') ?? 'FORWARD'),
            ...
            Log::info('Mikrotik getFirewallRules result', ['routeur_id' => $routeur->id, 'count' => count($rules)]);
            return $rules;
        } catch (\Throwable $e) {
    ...
    {
        try {
            Log::info('Mikrotik addFirewallRule called', ['routeur_id' => $routeur->id, 'data' => $data]);
            ...
            Log::info('Mikrotik addFirewallRule response', ['routeur_id' => $routeur->id, 'response' => (string)$response]);
            return true;
        } catch (\Throwable $e) {
            Log::error('Mikrotik addFirewallRule failed', ['routeur_id' => $routeur->id, 'data' => $data, 'error' => $e->getMessage()]);
            return false;
        }
    }
